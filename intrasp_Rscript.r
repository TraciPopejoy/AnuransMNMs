...
# paths
PATH <- "./" #arc
PATH_out <- paste0(PATH, "intra_results/") # where to save the output
# traits used to parameterize the ectotherm function
traits <- read.csv(paste0(PATH, "intraspecific_variation_subset_APPENDIX.csv")) %>%
...
# completed points
 pts_done <- read.csv('intra_summarized_bodytemps.csv') %>% pull(rowid)

# points at which to run the microclimate model
pts_df<-read.csv(paste0(PATH, "points_to_run.csv")) %>%
	  filter( !(rowid %in% pts_done), 
	  grepl(focal_spp[1], spp_all)|grepl(focal_spp[2], spp_all)|grepl(focal_spp[3], spp_all)|grepl(focal_spp[4], spp_all)|
		grepl(focal_spp[5], spp_all)|grepl(focal_spp[6], spp_all)|grepl(focal_spp[7], spp_all)) # prioritize cells that have more than one species 
#npts<-nrow(pts_df)
...
# for loop that only runs a  model if the species is found in that grid cell
  for(spp in focal_spp){
    if(grepl(spp, pt$spp_all)){
      spp_traits_all <- traits %>% filter(species == spp) # using the traits for each species
      for(rn in 1:nrow(spp_traits_all)){
      spp_traits<-spp_traits_all[rn,]
      cat(paste('ectotherm model for', spp,'trait value Tpref',spp_traits["Tpref"], '\n'))
...
ecto_out<-bind_rows(ecto_out, bodytemps %>% mutate(rn, loc=spp_traits$verbatimLocality[1], src=spp_traits$traitSource[1]))}
gc()
}
}
...
eval_fork <- function(row, timeout=900){ # feed it a rownumber and the amount of time to wait until killing core
...
micro_out<-NULL
for(i in list.files('intra_results/', pattern='microloc', full.names = T)){
  micro_out<-bind_rows(micro_out,
                      read.csv(i))
}
write.csv(micro_out, 'intra_microclimate_info.csv', row.names=F)

clim_out<-NULL
for(i in list.files('intra_results/', pattern='ppt_dat', full.names=T)){
  clim_out<-bind_rows(clim_out,
		      read.csv(i))
}
write.csv(clim_out, 'intra_climate_data.csv', row.names=F)

ecto_out<-NULL
for(i in list.files('intra_results', pattern='bodytemps_sum.csv', full.names = T)){
  ecto_out<-bind_rows(ecto_out,
                      read.csv(i))
}

write.csv(ecto_out, 'intra_summarized_bodytemps.csv', row.names=F)
