...
```{r}
pts2run<-read.csv('intermediate_results/points_ran_2022-11-17.csv')
nrow(pts2run)
pts2run %>% filter(rowid == '10592')

library(ncdf4)
nc = nc_open("global_micro_climate/soilw.mon.ltm.v2.nc")
precip = list()
precip$x = ncvar_get(nc, "longitude")
precip$y = ncvar_get(nc, "latitude")

precip$z = ncvar_get(nc, "z", start=c(1))
?ncvar_get
```
