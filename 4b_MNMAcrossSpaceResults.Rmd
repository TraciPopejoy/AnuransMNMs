...
ecto_out <- read.csv('intermediate_results/summarized_bodytemps_221128.csv')
micro_out <- read.csv('intermediate_results/microclimate_info_221128.csv')
```
...
test.data %>% 
  ...
  geom_point(aes(x=lat, y=WT_ptmin, color=pltsp), alpha=.05)+
  ...
  scale_x_continuous('Latitude', breaks=c(25, 30, 35, 39))+
  theme_classic()+
...
plot_grid(
  ...
  ens.df %>% 
    ...
    ggplot()+geom_point(aes(x=value, y=wt_mean), alpha=0.03)+
