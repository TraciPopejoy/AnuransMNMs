...
ecto_out <- read.csv('intermediate_results/summarized_bodytemps.csv')
micro_out <- read.csv('intermediate_results/microclimate_info.csv')
```

This gives us data for `r length(unique(ecto_out$rowid))` randomly sampled, but occupied points across our study region.
...
We already see some differences among species with G. carolinensis generally having large warming tolerance and L. palustris having a smaller warming tolerance. We can compare those differences across our latitudinal gradient using the data we have collected.

```{r lat wt plot with raw data, fig.width=4, fig.height=4, echo=F}
test.data %>% 
  ggplot()+
  geom_hline(yintercept=0, linetype='dashed')+
  geom_smooth(aes(x=lat, y=WT_ptmin, color=pltsp, linetype=pltsp), 
              method='lm', formula= 'y~x', level=.0)+
  geom_point(aes(x=lat, y=WT_ptmin, color=pltsp), alpha=.1)+
  scale_color_manual('species', 
                     values=met.brewer('Renoir'),
                     aesthetics = c('fill','color')) +
  scale_linetype_manual('species', values=c(1,2,1,2,1,2,1,2,1,2,1,2,1), guide='none')+
  scale_y_continuous('Warming Tolerance')+
  scale_x_continuous('Latitude')+
  theme_classic()+
  guides(colour = guide_legend(override.aes = list(linetype = c(1,1,2,1,2,1,2,1,2,1,2,1))))

ggsave('results/FigS3_lat_tsm_raw.jpg', width=4, height=4)
```

The lines above are simple linear models, unique to each species. We see that species vary in their estimated warming tolerance a lot and in their relationship between latitude and warming tolerance. The most extreme we can spot are G. carolinensis and L. catesbeianus, which have flat lines. G. carolinensis buries and L. catesbeianus 
...
```{r prior distributions, echo=F, fig.width=6.5, fig.height=2}
plot_grid(ggplot()+
...
```{r build plotting df lat reg, warning=F}
...
So it looks like there is a `r round((1-ecdf(lat.mcmc.df$beta.l)(0))*100, 1)`% probability (given our model and data), that range size is positively related to the slope in warming tolerance (or how quickly warming tolerance shifts across a species range). So there might be some support for Brattstrom's rule. 
...
test.data %>% 
  ggplot()+
  geom_vline(xintercept = c(0,2), color='lightgrey')+
...
ggsave('results/Fig2_cumdist.jpg', width=3.5, height=4.5)
```
...
So there are `r nrow(ens.df)` points that have more that two anuran species occurring there. 
...
plot_grid(
  ...
  ens.df %>% 
    ...
    ggplot()+geom_point(aes(x=value, y=wt_mean), alpha=0.3)+
...
We might have some support for the temperate sensitivity hypothesis. 

```{r}
model_string_ens<- "model{
  # Data model #
  for(i in 1:n.pts){
    tsm[i] <- beta*(lat[i]-latmean)+alpha
    E.tsm[i] ~ dnorm(tsm[i], tau.tsm[i])
  }
  beta ~ dnorm(0.0, 1e-6)
  alpha ~ dnorm(0.0, 1e-6)
}"
ens.model <- jags.model(textConnection(model_string_ens), 
                        data=list(E.tsm = ens.df$wt_mean,
                                  tau.tsm = ens.df$wt_sd,
                                  lat = ens.df$lat,
                                  latmean=mean(ens.df$lat),
                                  n.pts=nrow(ens.df)),
                        n.chains=3, n.adapt=500)
  update(ens.model, 500) 
  ens.mcmc<-coda.samples(ens.model,
                         variable.names = c("beta","alpha"),
                         n.iter=1000, thin=3)
summary(ens.mcmc)
gelman.diag(ens.mcmc)
```
