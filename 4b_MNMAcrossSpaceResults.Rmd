...
ecto_out <- read.csv('intermediate_results/summarized_bodytemps_011223.csv') %>%
  filter(species %in% mnm_spp)
micro_out <- read.csv('intermediate_results/microclimate_info_011223.csv')
```
...
traits.df %>%
  ...
  pivot_longer(-spF) %>%
  mutate(traitF=factor(name, levels=c('AOO','s.fossorial','CTmax','Tpref','Tmerge'))) %>%
ggplot()+
...
test.data %>% 
  ...
  #geom_point(aes(x=lat, y=WT_ptmin, color=pltsp), alpha=.03)+
  geom_smooth(aes(x=lat, y=WT_ptmin, color=pltsp), 
              method='lm', formula= 'y~x', level=.0, size=1.5)+
  geom_smooth(aes(x=lat, y=WT_ptmin, linetype=pltsp), 
  ...
  guides(fill=guide_legend(override.aes=list(alpha=0), ncol=3),
         color=guide_legend(ncol=3))

ggsave('results/Fig3_wt_lat.jpg', width=3.5, height=4)
```
...
model.data<-test.data %>% filter(species != 'Lithobates areolatus')
test.mod<-lm(WT_ptmin~lat:species + species + 0, data=model.data)
