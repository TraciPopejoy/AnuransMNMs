...
library(tidyverse);library(sf);library(stars); library(knitr)
library(ggrepel); library(cowplot); library(ggspatial)
library(NicheMapR)
se<-st_as_sf(readRDS('../CompareAnuranCCVA/spatial_extent.rds')) %>%
  st_make_valid() %>%
...
traits<-read.csv('input_data/inputed_physio_traits_011923.csv') # traits used throughout our analyses
...
pt_locs %>% 
  mutate(verbatimLocality=recode(verbatimLocality, 'Long Island, NY'='New York, Long Island')) %>%
...
pt_locs %>% 
  ...
  distinct(species, verbatimLocality, .keep_all = T) %>%
  st_as_sf(coords=c('longitude','latitude'), crs=st_crs("+proj=longlat +ellps=WGS84 +datum=WGS84")) %>%
  st_filter(st_transform(st_buffer(se,10), st_crs(.)))
```
...
```{r }
pt.warmtol<-pt.bt.df %>%
  select(species, rowid, CTmax, year, WT_ptmin, WT_min25) %>%
   pivot_longer(cols=c('WT_min25','WT_ptmin')) %>%
  mutate(nm.F=factor(name, levels=c('WT_ptmin','WT_min25'),
                     labels=c('minimum','hottest quarter'))) %>%
  group_by(species, rowid, CTmax, nm.F) %>%
...
```{r}
pt.WTord<-pt.warmtol %>% group_by(species) %>% filter(nm.F=='minimum') %>%
  summarize(mWTmin=mean(medWT)) %>%
  select(species, mWTmin) %>% arrange(mWTmin) %>% pull(species)
pt.warmtol %>%
  mutate(spF=factor(species, levels=rev(pt.WTord))) %>%
  ggplot()+
  geom_hline(yintercept=0, linetype='dashed')+
  geom_linerange(aes(x=spF, ymin=WTmin2, ymax=WTmin97, color=CTmax, group=CTmax),
             position=position_dodge(width=.7))+
  geom_point(aes(x=spF, y=medWT, color=CTmax, group=CTmax), size=1.7,
             position=position_dodge(width=.7))+
  scale_y_continuous('Daily minimum warming tolerance', labels = tmplab)+
  scale_color_gradient(low='lightgrey',high='black', labels = tmplab)+
  theme_classic()+
  ...
  coord_flip()+
  facet_grid(~nm.F, scales='free')
ggsave('results/FigS1_2_wt_results.jpg', width=5.5, height=6)
```


```{r}
sp.medWT<-pt.warmtol %>% select(species, rowid, CTmax, nm.F, medWT) %>% 
  pivot_wider(names_from=nm.F, values_from=medWT) %>%
  group_by(species) %>% summarize(across(c('minimum','hottest quarter'), median), .groups='drop') %>%
  left_join(read.csv('results/pt_wtmin_sum.csv') %>% filter(nm.F != 'median') %>%
              select(species, nm.F, medWT) %>% pivot_wider(names_from=nm.F, values_from=medWT) %>%
              rename(reg_min=minimum, reg25=`25th quartile`), by='species')
cor.test(sp.medWT$minimum, sp.medWT$reg_min, method='spearman')
cor.test(sp.medWT$`25th quartile`, sp.medWT$reg25, method='spearman')
sp.medWT %>% pivot_longer(-species) %>% mutate(type=factor(ifelse(grepl('hot',name)|grepl('25', name), 'hottest quarter', 'minimum'), levels=c('minimum','hottest quarter')),
                                              area=ifelse(grepl('reg', name), 'regional','acclimatization-control')) %>% select(-name) %>% 
  pivot_wider(names_from=area) %>%
  ggplot()+
  geom_point(aes(x=`acclimatization-control`, y=regional))+facet_wrap(~type, scales='free') + theme_classic()+
  theme(panel.background = element_rect(color='black', fill=NA),
        strip.background = element_blank(),
        axis.text=element_text(size=8))+
  scale_x_continuous(labels=tmplab, 
                     breaks=seq(0,15,by=5))+scale_y_continuous(labels=tmplab,breaks=seq(0,15, 5))
ggsave('results/FigS1_3_corWT.jpg', width=4, height=2)
```
