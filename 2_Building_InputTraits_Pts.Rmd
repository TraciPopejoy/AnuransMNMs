...
new_trait_mean<-#read.csv('G:/Shared drives/GCC Vulnerability Scaling Project/3_Physiological Trait Database/Data Release/ATraiU2_full_2022SEPT.csv') %>% 
  read.csv('C:/Users/Owner/Downloads/TRAD_full_2022SEPT.csv') %>%
...
Using the code below, we identified Tpref as a way to investigate intraspecific trait variation's effect on MNMs. Both Tmerge, Tpref and CTmax had some multiple recorded values for species in our species list. Of the traits we varied in our mechanistic niche model, Peterson and Gade (2017) found that optimal temperature had the largest effect on MNM outputs and it does not directly affect the interpretation of warming tolerance. Thus, we chose four species to vary Tpref (values observed from the literature) in our MNM models. 
...
trait_tree<-Rphylopars::left_join(tree.pruned, trait.df, by='node')
...
Reviewer requested some test of imputation consistency. 
```{r phyloreg loo}
# Code from https://github.com/ericgoolsby/Rphylopars/issues/51

# imputed_ind = individual-level imputations
# imputed_sp = species-level imputations
imputed_sp <- trait_data <- data.frame(trait.mat)
imputed_sp[,-1] <- NA
for(i in 1:nrow(trait_data))
{
  for(j in c(1:4, 8))
  {
    ...
    p_temp <- phylopars(trait_data = temp_trait_data,
                        tree = force.ultrametric(tree.pruned),
                  model='OU', phylocov_fixed = p_OU$pars$phylocov)
...
data.frame(traits=colnames(trait_data)[-1],
          "sp bias"=round(sp_mean_abs_error,3),
          "sp error" = round(sp_mean_sq_error,3),
          n = colSums(!is.na(trait_data[,-1])), 
          minimum=sapply(trait_data[,-1], min, na.rm=T),
          maximum=sapply(trait_data[,-1], max, na.rm=T))

imputed_sp[,2] - trait_data[,2]
```
