...
Using the phylogeny from Jetz & Pyron 2019, we can impute traits for the species we have trait information for. To improve the fit of this imputation, for our focal species, we are recalculating means that remove traits from far localities (e.g., Canada). 
...
trait_filtL<- trait_filt %>%
  mutate(ID=case_when(grepl('Texas', verbatimLocality) ~ 'texas',
...
usa_traits %>%
  ...
  theme_void()
ggsave('results/FigS2_traits_removed.jpg', width=4, height=2.75)
sf::sf_use_s2(TRUE)
```

So we removed `r nrows(new`

```{r}
nrow(new_trait_mean)-nrow(trait_filt)
anti_join(new_trait_mean, trait_filt) %>% ungroup() %>% count(species)
trait_filt %>% ungroup() %>% count(species) %>% summarize(mean(n), sd(n))
new_trait_mean
```


I did this based on the localities' state location distance from the southeast region.
...
```{r}
edge_sts2<-se %>% st_make_valid() %>% st_union()
dm2<-usa_traits %>% filter(!is.na(n_traits),
                      !(ID %in% se$ID)) %>%
  st_distance(edge_sts2)
rownames(dm2)<-usa_traits %>% filter(!is.na(n_traits), !(ID %in% se$ID)) %>% pull(ID)
dm2[order(dm2[,1]),]
```
...
both_trait_types %>%
  ...
  mutate(keptint=case_when(is.na(mean_val)& !is.na(value)~'inter', 
                           !is.na(mean_val) ~ 'se')) %>%
  filter(!is.na(keptint)) %>% count(keptint)
%>%
  select(species, traitName, trait_value) %>%
  pivot_wider(names_from=traitName, values_from=trait_value)
```
...
So based on the missing trait data of our focal species, we should either use Rphylopars or BHPMF + phylogeny to impute the missing trait data. I am going to use all the trait data available across the entire Southeast US anuran species group (n = 37 species) to impute the data using Rphylopars.

## Use Rphylopars to imput continuous trait data

```{r trait mat, warning=F, message=F, echo=F}
trait.mat<-trait.df[, c(17, 6:10, 12:14)] %>%
...
#results!
imp_results <- p_lamda$anc_recon[1:length(tree.pruned$tip.label),] %>%
  as_tibble() %>%
  bind_cols(species=rownames(p_lamda$anc_recon[1:length(tree.pruned$tip.label),])) %>%
...
imp_results %>% 
  ...
  filter(species.y %in% focal_spp) %>% count(Imputed)
imp_results %>% 
  left_join(taxa_table, by=c('species'='Tree.name')) %>%
  ...
  summarize(summary(per))
imp_results %>% 
  left_join(taxa_table, by=c('species'='Tree.name')) %>% 
  filter(species.y %in% focal_spp) %>% count(family, genus)
```
...
write.csv(used_traits, 'input_data/inputed_physio_traits_11152022.csv', row.names=F)

# writing out all imputed traits to explore options of expanding comparison species list 
p_lamda$anc_recon[1:length(tree.pruned$tip.label),] %>%
   as_tibble() %>%
  ...
  write.csv('input_data/MNM_traits_SEspp.csv', row.names = F)
```
...
se_grid<-NULL
focal_spp<-read.csv('AnuranMNMs/inputed_physio_traits.csv') %>% pull(species)
for(s in focal_spp){
  spp_aoo<-read_sf('RCS_resub/L48_1km', layer=s) %>%
    st_buffer(4) 
...
write.csv(se_pts_df, paste0('points_ran_', format(Sys.Date()), '.csv'), row.names = F)
```
...
rm(iucn_all, iucnrng, latlist_occs, na_r)
```
