...
all_trait_shts_mod<-all_trait_shts %>%
  ...
  filter(Trait %in% unique(all_trait_shts$Trait)[-c(8,11:16)], 
         !is.na(Value), # remove empty rows
         # remove non-published sources
         `Source file name` != 'Animal Diversity Web') 
```

Then we load our tangentally collected traits. These include traits from chapter 8 and chapter 9 of Felden & Bru 199, traits listed in references found in the above taxa-focused search, and those identified within Herpetological Review 196 - 2012 issues. 
...
I used the below code to identify references with doi and cross reference to populate author and journal information. I haven't used this code chunk since summer 2021 though. After Summer 2021, I looked up the rest of the references by hand. I also collected all the reference information from the Herpetelogical Review.
...
refs<-read.csv('../ATraiU2_reference_list_2022MAY.csv') %>% 
  group_by(citation) %>% 
  mutate(group_id=cur_group_id()) %>%
  rename(refID=group_id) %>%
  ungroup()
write.csv(refs, '../ATraiU2_reference_list_2022MAY.csv', row.names = F)
```
```{r print refs, echo=F}
refs<-read.csv('../ATraiU2_reference_list_2022MAY.csv') %>% select(-X)
...
full_join(tchar, tbif, by=c('species','lifeStage')) %>%
  ...
  write.csv('../ATraiU2_summary_values_2022MAY.csv')
```
```{r, echo=F}
...
trait_long %>%
  ...
  write_csv('../ATraiU2_full_2022MAY_2.csv')
```