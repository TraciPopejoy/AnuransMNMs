...
```{r}
trait_long<-read_csv('../ATraiU2_full_2022JULY.csv', show_col_types = F) %>%
...
```
Calculations for taxa outside our focal taxa list
```{r}
trait_long %>% filter(!(species %in% foc_taxa)) %>% nrow() 
trait_long %>% filter(!(species %in% foc_taxa)) %>% pull(species) %>% unique() %>% length()
trait_long %>% filter(!(species %in% foc_taxa)) %>% 
  distinct(species, traitName) %>% group_by(species) %>% 
...
We then investigated the distribution of reported trait values across life stages in the table below. Of those labelled adult, `r trait_long %>% filter(lifeStage=='unknown') %>% nrow()`. 
