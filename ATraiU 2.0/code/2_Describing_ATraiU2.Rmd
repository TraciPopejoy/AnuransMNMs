...
```{r}
trait_long<-read_csv('../ATraiU2_full_2022AUG1.csv', show_col_types = F) %>%
...
mk_year
n_t_year %>% filter(max(n)==n)
n_t_year %>% ggplot(aes(x=year, y=n))+geom_point()+
...
```
```{r}
trait_long %>% filter(is.na(traitEvidenceCategory), 
                      traitName =='Tpref')
trait_long %>% filter(traitSource == 'Noland1981_CTmax')
```
## Trait Coverage
