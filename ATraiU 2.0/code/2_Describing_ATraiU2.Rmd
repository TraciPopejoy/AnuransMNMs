...
trait_long %>% filter(lifeStage%in% c('adult','unknown')) %>%
  filter(species %in% c('Pseudacris fouquettei','Pseudacris brachyphona')) %>%
...
trait_hist<-trait_ref_ndf %>%
  ...
  scale_x_continuous(breaks=seq(0,35,5), 
                     expand=c(0.01,0.01), limits = c(0,35))+
  theme_classic()+
...
First I am going to get the phylopic information down through the API.
