...
```{r}
trait_long %>%
  mutate(type=ifelse(traitName %in% c("Mass","CTmax","CTmin","Tpref"), 'phys','behavior')) %>%
  count(type) %>%
  mutate(per=n/sum(n))
```
...
```{r sum stats, echo=F}
kable(trait_long %>% filter(species %in% foc_taxa) %>% count(traitName) %>% arrange(desc(n)),
...
We then investigated the distribution of reported trait values across life stages in the table below. 
...
And finally, we investigated how conservation status might influence trait completeness. Below we investigate which species in our focal taxa group are of conservation concern for the IUCN and for the National Species of Greatest Conservation Need list. None of our focal species are included on the ESA. 
...
spocc<-rcs %>% filter(species %in% foc_taxa, IUCNcategory == 'NT') %>%
  select(species, IUCNcategory, NatSGCN)
kable(spocc)
```
...
library(maps);library(sf); library(MetBrewer)
...
trait_hist<-trait_ref_ndf %>%
  ...
  draw_plot(se_inset, x=17.5, y=12, scale=15)+
...
Based on the trait data, I think total possible trait completenes for all life stages would be `r 9+2+4+2+5`.
...
```{r, echo=F}
kable(trait_ls_total %>% filter(n_ls > 2) %>% select(species, n, total_ls),
      caption='Species with more than two life stages with trait values available')
kable(trait_ls_total %>% 
  mutate(n_ls_grp=ifelse(n_ls > 2 & !is.na(n_ls), 'more', 'less')) %>% 
  group_by(n_ls_grp) %>% summarize(min(AOO_WS_sqkm)),
  caption='minimum AOO of species with > 2 life stages with trait values reported or < 2 life stages with trait values reported')
```
