...
# seeing if within grid matters that much 
bind_rows(
pt1_micro<-micro_usa(loc=c(-77.2233, 37.1507))
pt2_micro<-micro_usa(loc=c(-77.2081, 37.1342))
library(lubridate)
bind_rows(pt1_micro$metout %>% as_tibble() %>% mutate(name='point 1') %>%
            rowid_to_column(),
          pt2_micro$metout %>% as_tibble() %>% mutate(name='point 2') %>%
            rowid_to_column()) %>%
  filter(DOY==1, rowid > 8000) %>% 
  ggplot()+
  ...
  ggtitle('January 1st, 2016; VA grid 539')+
  guides(colour = guide_legend(override.aes = list(alpha = 1)))
st_sfc(st_point(c(-77.2233, 37.1507)),
          st_point(c(-77.2081, 37.1342)),
       crs=4326) %>%
  st_transform(st_crs("+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=km +no_defs")) %>%
  st_distance()
