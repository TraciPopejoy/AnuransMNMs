...
test.data %>%
  ...
  geom_sf(aes(geometry=geometry, color=tsm, fill=tsm))+
  facet_wrap(~species)+
...
test.data %>% # CHANGE THIS!!!
  ...
  geom_smooth(aes(x=Latitude, y=tsm, color=pltsp, linetype=pltsp), method='lm', formula= 'y~x', level=.0)+
  geom_point(aes(x=Latitude, y=tsm, color=pltsp), alpha=.5)+
...
plot_grid(lat.reg.out %>%
  filter(stat =='beta', !(parameter %in% c('beta.l','alpha.l')))  %>%
  ggplot()+
  geom_linerange(aes(x=rng, ymin=`2.5%`, ymax=`97.5%`, color=species))+
  geom_point(aes(x=rng, y=`50%`, color=species))+ 
  scale_color_manual('species', values=met.brewer('Renoir'),
                     aesthetics = c('fill','color')) +
...
lat.reg.out %>% 
  filter(stat %in% c('alpha','beta'), !(parameter %in% c('beta.l','alpha.l')))  %>%
  dplyr::select(stat, species, `2.5%`,`50%`,`97.5%`) %>%
  pivot_wider(names_from=stat, values_from=c(`2.5%`,`50%`,`97.5%`))%>%
  rename(b0=`50%_alpha`, b1=`50%_beta`) %>%
  ggplot() +
  geom_point(data=test.data, aes(x=Latitude-mean(Latitude), y=tsm, color=factor(species)))+
  geom_abline(aes(slope=b1, intercept = b0, color=factor(species))) +
  xlim(min(test.data$Latitude)-mean(test.data$Latitude)-5,
       max(test.data$Latitude)-mean(test.data$Latitude)+5) +
  ylim(0, 10) 
```
...
test.data %>% 
  ...
  geom_vline(xintercept = c(0,2), color='lightgrey')+
    stat_ecdf(aes(x = tsm, color = pltsp), lwd=3, geom = "step", alpha=0.7)+
  stat_ecdf(aes(x = tsm, linetype=pltsp), geom = "step", alpha=0.9)+
