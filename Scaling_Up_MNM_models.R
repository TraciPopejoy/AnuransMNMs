...
anax<-read.csv('../National-RCS/data/occ_data/Anaxyrus americanus_20210330.csv') %>%
  st_as_sf(coords=c('Longitude','Latitude'), crs=st_crs("+proj=longlat +ellps=WGS84 +datum=WGS84"))
anax_1per<-st_join(anax,
          va_r %>% 
...
library(NicheMapR)
write.csv(va_pts, 'micro_climate_outputs/points_run.csv')
...
for(i in 1:3){
  ...
  micro<-micro_usa(loc = c(va_pts[i,1],va_pts[i,2]),
                 ...
                 Usrhyt = 0.01)
  # save the output for ease of access later
  saveRDS(micro, paste0('micro_climate_outputs/PT_row',
                        i,'.rds'))
...
plot_df<-ecto_out %>% 
  left_join(va_pts %>% as_tibble() %>% rowid_to_column('point'), ) %>%
